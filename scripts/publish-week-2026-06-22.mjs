...
async function ensureNoExistingWeeks(supabase, resolved) {
  ...
  const { data, error } = await supabase.from("weeks").select("id, student_id, label, status")
    .in("student_id", ids).eq("date_start", WEEK.dateStart).eq("date_end", WEEK.dateEnd).in("status", ["draft", "published"]);
...
async function main() {
  ...
  if (shouldPublish && !serviceRole) {
    throw new Error("Publicacao em producao exige SUPABASE_SERVICE_ROLE_KEY para auditar duplicidades sem depender de RLS.");
  }
